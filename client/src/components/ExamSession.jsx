  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sectionStudents = studentRes.data.filter(s => s.Section === sectionId);
      ...
      <div className="bg-white border-b sticky top-0 z-40">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900">{exam?.course}</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20">Section {sectionId}</Badge>
                <span className="text-xs text-slate-400 font-medium tracking-wider uppercase">{exam?.year}</span>
              </div>
            </div>
          </div>
          <Button onClick={handleSubmit} className="gap-2 bg-primary shadow-lg shadow-primary/20">
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50/30">
                  ...
                  <TableHead className="w-[200px]">Activity / Incident</TableHead>
                  <TableHead>Remarks</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {students.map((student) => (
                  <TableRow key={student.RegNO} className={!attendance[student.RegNO] ? 'bg-red-50/30' : ''}>
                    <TableCell className="text-center">
                      <Checkbox 
                        checked={attendance[student.RegNO]} 
                        onCheckedChange={(val) => setAttendance(prev => ({...prev, [student.RegNO]: val}))}
                        className="mx-auto"
                      />
                    </TableCell>
                    <TableCell className="font-mono text-xs font-bold text-slate-500">{student.RegNO}</TableCell>
                    <TableCell>
                      <div className="font-bold text-slate-800">{student.Name}</div>
                      <div className="text-[10px] text-slate-400 font-medium">{student.Specialization}</div>
                    </TableCell>
                    <TableCell>
                      <Select 
                        value={activities[student.RegNO]} 
                        onValueChange={(val) => setActivities(prev => ({...prev, [student.RegNO]: val}))}
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="Bit Material">Bit Material</SelectItem>
                    ...
                    <TableCell>
                      <Input 
                        ...
                        onChange={(e) => setRemarks(prev => ({...prev, [student.RegNO]: e.target.value}))}
